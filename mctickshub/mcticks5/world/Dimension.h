...
void setBlockID(const BlockPos& pos, int blockID) {
        ...
        // --- 调试：只在开始时打印一次 ---
        // fprintf(stderr, "[DEBUG] Dimension::setBlockID: START. Pos=(%d, %d, %d) NewID=%d\n",
        ...
        if (oldLightEmission != newLightEmission) {
            // --- 调试：仅在触发光照更新时打印 ---
            fprintf(stderr, "[DEBUG] Dimension::setBlockID: Queuing light change at (%d,%d,%d). OldEmit=%d, NewEmit=%d\n",
                    pos.x, pos.y, pos.z, oldLightEmission, newLightEmission);
            addLightChange(pos, oldLightEmission, newLightEmission);
        }

        // --- 调试：移除结束日志 ---
        // fprintf(stderr, "[DEBUG] Dimension::setBlockID: END. Pos=(%d, %d, %d)\n", pos.x, pos.y, pos.z);
    }
private:
