...
private:
    ...
    void runCollectedTicks(std::function<void(BlockPos, LevelTicks&)> consumer, std::function<void(BlockPos, int)> setBlockID) {
        ...
        // 将 std::queue<ScheduledTick> tempQueue 改为 std::vector<ScheduledTick>，以便进行并行迭代
        ...
        // 4. 并行化 consumer 执行循环 (替换原有的 while (!tempQueue.empty()) 循环)
        #pragma omp parallel for schedule(guided)
        for (size_t i = 0; i < ticksToProcess.size(); ++i) {
            auto tick = ticksToProcess[i];
